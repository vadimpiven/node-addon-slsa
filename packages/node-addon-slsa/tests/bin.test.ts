...
/**
 * End-to-end tests for the `slsa` CLI bin: spawns the built
 * `bin/slsa.mjs` as a child process and asserts exit codes, usage
 * output, debug-flag gating, error surfacing, and that the published
 * `dist/` bundle has no external imports (runtime or type).
 */
...
describe("slsa bin", () => {
  ...
  it("error output includes stack trace", async ({ expect }) => {
    await using tmp = await tempDir();
    await writeFile(join(tmp.path, "package.json"), "{}");
    const { code, stderr } = await run(["wget"], tmp.path);
    expect(code).toBe(1);
    expect(stderr).toMatch(/\s+at\s+/);
  });

  it("ProvenanceError (missing manifest) reports the underlying cause", async ({ expect }) => {
    await using tmp = await tempDir();
    await writeTestPkg(tmp.path, "1.0.0");
    const { code, stderr } = await run(["wget"], tmp.path);
    expect(code).toBe(1);
