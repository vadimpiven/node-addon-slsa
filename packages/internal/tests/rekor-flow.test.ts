...
/**
 * Asserts that `verifyRekorAttestations` throws an aggregate
 * ProvenanceError when every candidate entry fails sigstore
 * verification. Real Rekor JSON, real Fulcio cert bytes, real undici
 * HTTP via MockAgent; an injected `BundleVerifier` stub forces every
 * verification to throw — no TUF / Fulcio-specific state required.
 */
...
// Body from a real Rekor entry (cli/cli). X509Certificate.parse accepts it;
// the cert's OIDs are irrelevant because the stubbed Verifier throws first.
...
// Build an `attestation.data` field whose in-toto Statement subject digest
// matches SEARCH_SHA, so parseRekorEntry's subject-digest binding succeeds
// and the flow reaches the stubbed Verifier.
